<div class="row">
	<div class="col-md-12">
		<legend class="red_legend edit <?php echo isset($s3_ffup) && $s3_ffup ? 'super_indented' : ''; ?>">Follow Up History</legend>
	</div>
	...
	<div class="col-md-11 col-md-offset-1">
		<table id="review_history_tbl" class="table table-bordered table-striped">
			...
			<tbody>
				...
				?>
							<tr>
								<td class="center ff-date" data-id="<?php echo $ff_up['id']; ?>"><div class="cell-datepicker"><?php echo formatDateForDisplay(isset($ff_up['ff_date']) ? $ff_up['ff_date'] : ''); ?></div></td>
								...
								<td class="center next-ff-date" data-id="<?php echo $ff_up['id']; ?>"><div class="cell-datepicker"><?php echo formatDateForDisplay(isset($ff_up['next_ff_date']) ? $ff_up['next_ff_date'] : ''); ?></div></td>
									...
									<?php 
						  			...
						  			if(!($filenames == null || empty($filenames))) {
						  				foreach ($filenames as $file):
						  					if(isset($s3_ffup) && $s3_ffup) {
							  					$url = '/file/get_s3/' . $file->filename;
						  					} else {
							  					$url = '/file/get/' . $file->filename;
						  					}
						  		?>
							  				<a href="<?php echo $url; ?>" class="<?php echo isset($s3_ffup) && $s3_ffup ? 'file_name_s3' : 'file_name'; ?>"><?php echo $file->orig_filename; ?></a></br>
						  		<?php 
						  				endforeach;
						  			}
						  		?>
