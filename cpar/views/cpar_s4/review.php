<script src="/_js/cpar_s4/edit.js"></script>
...
<?php if($can_edit_cpar || ($this->session->userdata('logged_in_user') && $this->session->userdata('logged_in_user')->access_level == ACCESS_LEVEL_ADMIN_FLAG)) { ?>
  ...
  <div class="container">
	...
	<?php 
		$this->load->view('partials/steps', array(
													'step_title' => 'Stage 4 <span class="de-em">CPAR</span>', 
    ...
    <form id="cpar_form" class="form-horizontal basic_form" enctype="multipart/form-data">
					...
					<table id="task_tbl" class="table table-bordered table-striped">
						...
						<tbody>
							<tr class="no_added_tasks">
								<td colspan="9" class="center">No added tasks.</td>
							</tr>
						</tbody>
					</table>
				</div>
	    ...
	    <?php	    	
	    	if($can_edit_cpar) {
					...
					if((int)$cpar->sub_status == CPAR_MINI_STATUS_S4_4A || (int)$cpar->sub_status == CPAR_MINI_STATUS_S4_4A2) {
						require_once('partial/_ims_review.php');
					} else if((int)$cpar->sub_status == CPAR_MINI_STATUS_S4_4B) {
						require_once('partial/_ims_review_4b.php');
					}
	    	} else {
					
	    		if((int)$cpar->sub_status == CPAR_MINI_STATUS_S4_4B && (int)$access->mr_flag == MR_FLAG) {
	    			require_once('partial/_ims_review_4b.php');
	    		}
	    	}
	    ?>
	    ...
	    <div class="edit_buttons_container row">
	    	<div class="col-md-12 button_div">
				...
				&nbsp;
				<!--// Export PDF Button -->	    	
	    		<?php if($cpar->status == CPAR_STAGE_4 && (strcmp($cpar->sub_status, CPAR_MINI_STATUS_S4_4A) == 0 || strcmp($cpar->sub_status, CPAR_MINI_STATUS_S4_4A2) == 0) && $can_edit_cpar) { ?>
	    			<a id="proceed_btn" class="submit_btn btn btn-primary"><i class="glyphicon glyphicon-floppy-disk"></i>&nbsp;Proceed</a>
			      &nbsp;
	    		<?php } ?>
	    		<?php if($cpar->status == CPAR_STAGE_4 && strcmp($cpar->sub_status, CPAR_MINI_STATUS_S4_4B) == 0 && ($can_edit_cpar || (int)$access->mr_flag == MR_FLAG)) { ?>
	    			<a id="proceed_4b_btn" class="submit_btn btn btn-primary"><i class="glyphicon glyphicon-floppy-disk"></i>&nbsp;Proceed</a>
			      &nbsp;
	    		<?php } ?>
